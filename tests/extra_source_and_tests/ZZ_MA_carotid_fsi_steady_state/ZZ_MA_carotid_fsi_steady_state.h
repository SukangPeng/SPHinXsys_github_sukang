...
Real length_scale_vessel = 1.0;                                           /**< Length scale factor, dimensionless (no unit) */
Real length_scale = pow(10, -3);                                          /**< Length scale factor, dimensionless (no unit) */
Real resolution_ref = 0.4 * length_scale;                                 /**< Initial reference particle spacing, unit: meters (m) */
Real BW = resolution_ref * 4.0;                                           /**< Reference size of the emitter, unit: meters (m) */
Vec3d domain_lower_bound = Vec3d(-8.0, -6.0, -35.0) * length_scale;       /**< Lower boundary of the system domain, unit: meters (m) */
Vec3d domain_upper_bound = Vec3d(18.0, 15.0, 30.0) * length_scale;        /**< Upper boundary of the system domain, unit: meters (m) */
BoundingBox system_domain_bounds(domain_lower_bound, domain_upper_bound); /**< Bounding box defining the system domain */
//----------------------------------------------------------------------
//	NormCalculator
//----------------------------------------------------------------------
...
// inlet 参数 inlet R=2.9293, (1.5611, 5.8559, -30.8885), (0.1034, -0.0458, 0.9935) A=24.95
// Real DW_inlet = 2.9293 * 2 * length_scale;
Real d_inlet = 6.3 * length_scale;
...
Vec3d inlet_cut_translation = Vec3d(1.5611, 5.8559, -30.8885) * length_scale - inlet_normal * 2.0 * resolution_ref;
Vec3d inlet_buffer_translation = Vec3d(1.5611, 5.8559, -30.8885) * length_scale + inlet_normal * 2.0 * resolution_ref;

Vec3d inlet_delete_translation = Vec3d(1.5611, 5.8559, -30.8885) * length_scale - inlet_normal * 5.0 * resolution_ref;
...
// outlet_large 参数 outlet_large R=1.9416, (-2.6975, -0.4330, 21.7855), (-0.3160, -0.0009, 0.9488) A=11.93
// Real DW_outlet_large = 1.9416 * 2 * length_scale;
Real d_outlet_large = 4.5 * length_scale;
...
Vec3d outlet_large_cut_translation = Vec3d(-2.6975, -0.4330, 21.7855) * length_scale + outlet_large_normal * 2.0 * resolution_ref;
Vec3d outlet_large_buffer_translation = Vec3d(-2.6975, -0.4330, 21.7855) * length_scale - outlet_large_normal * 2.0 * resolution_ref;

Vec3d outlet_large_delete_translation = Vec3d(-2.6975, -0.4330, 21.7855) * length_scale + outlet_large_normal * 5.0 * resolution_ref;
...
// outlet_small 参数 outlet_small R=1.3261, (9.0220, 0.9750, 18.6389), (-0.0399, 0.0693, 0.9972) A=5.22
// Real DW_outlet_small = 1.3261 * 2 * length_scale;
Real d_outlet_small = 3.0 * length_scale;
...
Vec3d outlet_small_normal_flow(0.0, 0.0, 1.3);
Vec3d outlet_small_cut_translation = Vec3d(9.0220, 0.9750, 18.6389) * length_scale + outlet_small_normal * 2.0 * resolution_ref;
Vec3d outlet_small_buffer_translation = Vec3d(9.0220, 0.9750, 18.6389) * length_scale - outlet_small_normal * 2.0 * resolution_ref;

Vec3d outlet_small_delete_translation = Vec3d(9.0220, 0.9750, 18.6389) * length_scale + outlet_small_normal * 5.0 * resolution_ref;
...
//----------------------------------------------------------------------
//  Global parameters for solid properties (vessel wall properties)
//----------------------------------------------------------------------
Real rho0_s_vessel = 1080;              /**< Density of the vessel wall, unit: kg/m³ */
Real poisson_vessel = 0.45;             /**< Poisson's ratio of the vessel wall (dimensionless) */
Real Youngs_modulus_vessel = 1e6;       /**< Young's modulus of the vessel wall, unit: Pa */
Real physical_viscosity_vessel = 100.0; /**< Physical viscosity of the vessel wall, unit: Pa·s */
//----------------------------------------------------------------------
...
class WallBoundary : public ComplexShape
{
  public:
    ...
    {
        add<ExtrudeShape<TriangleMeshShapeSTL>>(3.0 * resolution_ref, vessel_fluid_path, translation_wall_boundary, length_scale);
