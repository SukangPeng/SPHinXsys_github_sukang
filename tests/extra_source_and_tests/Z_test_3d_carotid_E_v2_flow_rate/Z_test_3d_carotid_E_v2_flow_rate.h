...
Real dp_0 = 0.4 * length_scale;                 /**< 初始参考粒子间距，单位：m (米) */
Real wall_resolution = dp_0;                    /*thickness = 1.0 * shell_resolution*/
Real BW = dp_0 * 4.0;                           /**< 发射器的参考大小，单位：m (米) */
Vec3d domain_lower_bound = Vec3d(-8.0, -6.0, -35.0) * length_scale; /**< 系统域的下边界，单位：m (米) */
Vec3d domain_upper_bound = Vec3d(18.0, 15.0, 30.0) * length_scale; /**< 系统域的上边界，单位：m (米) */
...
Real DW_inlet = 6.3 * length_scale;
Real A_inlet = 24.95E-6;
...
Real DW_outlet_large = 4.5 * length_scale;
Real A_outlet_large = 11.93E-6;
...
Real DW_outlet_small = 3.0 * length_scale;
Real A_outlet_small = 5.22E-6;
...
Real c_f = 10.0 * U_f * SMAX(Real(1), DW_inlet *DW_inlet / (DW_outlet_large * DW_outlet_large + DW_outlet_small * DW_outlet_small));
 ...
 * @brief 删除（切换为缓冲粒子）位于指定检测区域内的粒子
 *
 * 该类继承自 BaseLocalDynamics<BodyPartByCell>，
 * 使用 BodyAlignedBoxByCell 定义的检测区域（aligned_box），
 * 对位于该区域内的所有粒子调用 particles_->switchToBufferParticle(index) 进行删除处理。
 */
class DeleteParticlesInAlignedBoxByCell : public BaseLocalDynamics<BodyPartByCell>
{
  public:
    explicit DeleteParticlesInAlignedBoxByCell(BodyAlignedBoxByCell &aligned_box_part)
        : BaseLocalDynamics<BodyPartByCell>(aligned_box_part),
          ...
          aligned_box_(aligned_box_part.getAlignedBox()) // 新版本中，使用 getAlignedBox() 获得 AlignedBox 对象
    {
    }
    ...
    {
        // 使用 lock_guard 进行简单加锁，确保多线程情况下内存安全
        std::lock_guard<std::mutex> lock(mutex_);
        // 如果粒子在检测区域内且 index_i 是一个真实粒子，则切换该粒子为缓冲粒子
  ...
  protected:
    std::mutex mutex_;
    Vecd *pos_;               ///< 粒子位置数据
    AlignedBox &aligned_box_; ///< 检测区域的 AlignedBox 对象
};

//----------------------------------------------------------------------
//	FlowRateCalculator: 计算流体通过截面的瞬时流量
//----------------------------------------------------------------------
class FlowRateCalculator : public BaseLocalDynamicsReduce<ReduceSum<Real>, BodyPartByCell>
{
  protected:
    ...
    Real *physical_time_;
    std::string name_prefix_; // 传入的自定义前缀，如 "inlet", "outlet_large", "outlet_small"
  ...
  public:
    explicit FlowRateCalculator(BodyAlignedBoxByCell &aligned_box_part, Real outlet_area, const std::string &prefix)
        : BaseLocalDynamicsReduce<ReduceSum<Real>, BodyPartByCell>(aligned_box_part),
          ...
          name_prefix_(prefix) // 这里使用传入的 name_prefix 作为文件名
    {
    }
    ...
    Real reduce(size_t index_i, Real dt = 0.0)
    {
        ...
        return velocity_frame[alignment_axis_];
    }
    ...
    virtual Real outputResult(Real reduced_value) override
    {
        ...
        if (num_particles > 0)
        {
            average_velocity_ = reduced_value / Real(num_particles);
        }
        else
        {
            average_velocity_ = 0.0;
