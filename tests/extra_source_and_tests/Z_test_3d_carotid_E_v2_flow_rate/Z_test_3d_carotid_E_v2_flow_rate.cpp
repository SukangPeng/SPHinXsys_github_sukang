...
#include "Z_test_3d_carotid_E_v2_flow_rate.h"
#include "sphinxsys.h"
using namespace SPH;
//-----------------------------------------------------------------------------------------------------------
...
int main(int ac, char *av[])
{
    ...
    Vec3d observer_inlet_location = Vec3d(1.5611, 5.8559, -30.8885) * length_scale + inlet_normal * 5.0 * dp_0;
    Vec3d observer_outlet_large_location = Vec3d(-2.6975, -0.4330, 21.7855) * length_scale - outlet_large_normal * 5.0 * dp_0;
    Vec3d observer_outlet_small_location = Vec3d(9.0220, 0.9750, 18.6389) * length_scale - outlet_small_normal * 5.0 * dp_0;
    ...
    // Vecd oberver_point1 = Vecd(2.51680152 * length_scale, 5.20993528 * length_scale, -20.95315931 * length_scale);
    // StdVec<Vecd> observer_location1 = {oberver_point1}; /**< Displacement observation point. */
    // ObserverBody velocity_observer(sph_system, "VelocityObserver");
    // velocity_observer.generateParticles<ObserverParticles>(observer_location);
    ...
    Vec3d delete_half(3.0 * dp_0, 3.0 * dp_0, 3.0 * dp_0);
    ...
    BoundaryGeometryAlignedBox inlet_boundary(wall_boundary, "InletBoundary",
    AlignedBox(xAxis, Transform(Rotation3d(inlet_emitter_rotation), Vec3d(inlet_buffer_translation)), inlet_fix_half));

    BoundaryGeometryAlignedBox outlet_large_boundary(wall_boundary, "OutletLargeBoundary",
    ...
    BoundaryGeometryAlignedBox outlet_small_boundary(wall_boundary, "OutletSmallBoundary",
    ...
    //BodyAlignedBoxByCell left_disposer(water_block, AlignedBox(xAxis, Transform(Rotation3d(inlet_disposer_rotation), Vec3d(inlet_buffer_translation)), inlet_half));
    //SimpleDynamics<fluid_dynamics::DisposerOutflowDeletion> left_disposer_outflow_deletion(left_disposer);
    //BodyAlignedBoxByCell right_up_disposer(water_block, AlignedBox(xAxis, Transform(Rotation3d(outlet_large_disposer_rotation), Vec3d(outlet_large_buffer_translation)), outlet_large_half));
    //SimpleDynamics<fluid_dynamics::DisposerOutflowDeletion> right_up_disposer_outflow_deletion(right_up_disposer);
    //BodyAlignedBoxByCell right_down_disposer(water_block, AlignedBox(xAxis, Transform(Rotation3d(outlet_small_disposer_rotation), Vec3d(outlet_small_buffer_translation)), outlet_small_half));
    ...
    //ReduceDynamics<FlowRateCalculator> compute_inlet_flow_rate(inlet_emitter, PI * pow(DW_inlet / 2.0, 2), "inlet");
    //ReduceDynamics<FlowRateCalculator> compute_outlet_large_flow_rate(outlet_large_emitter, PI * pow(DW_outlet_large / 2.0, 2), "outlet_large");
    //ReduceDynamics<FlowRateCalculator> compute_outlet_small_flow_rate(outlet_small_emitter, PI * pow(DW_outlet_small / 2.0, 2), "outlet_small");
    ReduceDynamics<FlowRateCalculator> compute_inlet_flow_rate(inlet_emitter, A_inlet, "inlet");
    ReduceDynamics<FlowRateCalculator> compute_outlet_large_flow_rate(outlet_large_emitter, A_outlet_large, "outlet_large");
    ReduceDynamics<FlowRateCalculator> compute_outlet_small_flow_rate(outlet_small_emitter, A_outlet_small, "outlet_small");
    //----------------------------------------------------------------------
    //	Algorithms of FSI.
    //----------------------------------------------------------------------
    ...
    while (physical_time < end_time)
    {
        ...
        compute_inlet_flow_rate.exec();
        compute_outlet_large_flow_rate.exec();
        compute_outlet_small_flow_rate.exec();
    ...
    {
        write_point_velocity.generateDataBase(1.0e-3);
    }
    else
    {
        write_point_velocity.testResult();
    }
