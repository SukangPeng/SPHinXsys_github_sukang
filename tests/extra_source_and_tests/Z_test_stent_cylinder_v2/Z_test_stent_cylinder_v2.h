...
std::string vessel_wall_path = "./input/vessel_test_narrow.stl";
//----------------------------------------------------------------------
//	Basic geometry parameters
//----------------------------------------------------------------------
Vec3d translation_vessel_wall(0.0, 0.0, 0.0);                             /**< 血管壁的初始平移，单位：m (米) */
Vec3d translation_stent(0.0, 0.0, 0.0);                                   /**< 支架的初始平移，单位：m (米) */
Real length_scale = 1e-3;                                                 /**< 长度比例因子，无量纲 (无单位) */
Real length_scale_stent = 1e-3;                                           /**< 长度比例因子，无量纲 (无单位) */
Real length_scale_vessel = 1.0;                                           /**< 长度比例因子，无量纲 (无单位) */
Real resolution_ref = 0.15 * length_scale;                                /**< 初始参考粒子间距，单位：m (米) */
Real diameter = 0.004;                                                    /**< 血管外径，单位：m (米) */
Vec3d domain_lower_bound = Vec3d(-20.0, -5.0, -22.0) * length_scale;      /**< 系统域的下边界，单位：m (米) */
Vec3d domain_upper_bound = Vec3d(30.0, 10.0, 5.0) * length_scale;         /**< 系统域的上边界，单位：m (米) */
...
//----------------------------------------------------------------------
//	Global parameters for stent (支架材料参数)
//----------------------------------------------------------------------
Real rho0_s_stent = 6450.0;
Real poisson_stent = 0.33;
Real youngs_modulus_stent = 1e10; /**< 5 GPa，适中刚度 */
Real physical_viscosity_stent = 100000000.0;

Real yield_stress_stent = 8e7;
Real hardening_modulus_stent = 2e8;
Real saturation_flow_stress_stent = 2.5e8;
Real saturation_exponent_stent = 5.0;
//----------------------------------------------------------------------
//	Define SPH bodies.
//----------------------------------------------------------------------
...
class VesselWall : public ComplexShape
{
  public:
    ...
    {
        add<TriangleMeshShapeSTL>(vessel_wall_path, translation_vessel_wall, length_scale_vessel);
    }
};
//----------------------------------------------------------------------
//	BoundaryGeometry.
...
class BoundingBoxCalculator
{
  public:
    /**
     * @brief 获取 **全局坐标系** 下的 Bounding Box 并打印信息
     */
    static BoundingBox getGlobalBoundingBox(BaseParticles &particles, bool print_info = true)
    {
        Vecd lower_bound = particles.ParticlePositions()[0]; // 初始化为第一个粒子位置
    ...
    /**
     * @brief 获取 **支架自身坐标系** 下的 Bounding Box
     * @details 需要提供 **旋转矩阵** 和 **支架中心点**
     */
    ...
    {
        Mat3d inv_rotation = rotation_matrix.transpose(); // 计算 **逆旋转矩阵**
        Vecd inv_translation = -center;                   // 平移到 **局部坐标系**
...
{
  protected:
    Real magnitude_;        // 径向力大小
    Vecd center_;           // 支架中心
    Mat3d rotation_matrix_; // 旋转矩阵（全局 -> 局部）
    Vecd axis_direction_;   // 支架中心轴方向（全局）
  ...
  public:
        ...
        : magnitude_(magnitude), center_((pointA + pointB) * 0.5)
    {
        ...
        // **构造旋转矩阵，使得局部 X 轴与支架对齐**
        Vecd local_x = axis_direction_;
        Vecd local_z = Vecd(0, 0, 1);              // 假设 Z 轴为参考向量
        if (std::abs(local_x.dot(local_z)) > 0.99) // 防止与 Z 轴平行
        {
            local_z = Vecd(0, 1, 0);
        }
    ...
    Vecd InducedAcceleration(const Vecd &global_position, Real physical_time = 0.0) const
    {
        ...
        radial_direction[0] = 0.0; // **X 方向不施加力**
    ...
    {
        Vecd local_force = Vecd(0.0, magnitude_, magnitude_); // 局部 YZ 方向的力
...
class StartupRadialForce : public RadialForce
{
  ...
  public:
    ...
    Vecd InducedAcceleration(const Vecd &global_position, Real physical_time) const
    {
        ...
        Real sine_value = (time_factor < 1.0) ? sin(PI / 2 * time_factor) : 1.0; // **确保力逐渐增大**

        // **使用 `RadialForce` 计算径向方向，并乘以增长因子**
        return sine_value * RadialForce::InducedAcceleration(global_position, physical_time);
    }

    Vecd GetCurrentForce(Real physical_time) const
    {
        Real time_factor = physical_time / target_time_;
        Real sine_value = (time_factor < 1.0) ? sin(PI / 2 * time_factor) : 1.0;

        return sine_value * RadialForce::GetCurrentForce();
    }
};
...
class IncreaseToFullRadialForce : public RadialForce
{
  ...
  public:
    IncreaseToFullRadialForce(Real magnitude, const Vecd &pointA, const Vecd &pointB, Real time_to_full_force)
        : RadialForce(magnitude, pointA, pointB), time_to_full_force_(time_to_full_force) {}
    ~IncreaseToFullRadialForce() {}

    Vecd InducedAcceleration(const Vecd &global_position, Real physical_time) const
    {
        Real time_factor = physical_time / time_to_full_force_;
        Real scaling_factor = (time_factor < 1.0) ? time_factor : 1.0; // **线性增长，最大为 1.0**
...
class RadialForceApplication : public ForcePrior
{
  protected:
    ...
    // ✅ **新增变量**，用于存储部分粒子的调试信息
    std::vector<size_t> sampled_indices_; // 存储选中的粒子索引
  ...
  public:
          ...
          physical_time_(sph_system_.getSystemVariableDataByName<Real>("PhysicalTime"))
    {
        ...
        for (size_t i = 0; i < total_particles; i += std::max(1, (int)(total_particles / 10))) // 每 10% 选一个粒子
        {
...
class CustomContactForce : public solid_dynamics::RepulsionForce<SPH::Contact<>>
{
  public:
    ...
    {
        size_t total_particles = particles_->TotalRealParticles(); // 获取粒子数

        for (size_t index_i = 0; index_i < total_particles; ++index_i)
        {
            repulsion_force_[index_i] = Vecd::Zero(); // 确保初始化
