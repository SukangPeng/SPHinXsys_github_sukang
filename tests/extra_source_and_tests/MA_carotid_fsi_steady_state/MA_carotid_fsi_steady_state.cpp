 ...
 * @brief SPH-based simulation of blood flow in a carotid artery with fluid-structure interaction (FSI).
 * @details This file implements an advanced numerical simulation of pulsatile blood flow in a patient-specific
 *          carotid artery model using the Smoothed Particle Hydrodynamics (SPH) method within the SPHinXsys framework.
 * @author Sukang Peng
 * @date March 2025
 */
#include "MA_carotid_fsi_steady_state.h"
#include "sphinxsys.h"
using namespace SPH;
//-----------------------------------------------------------------------------------------------------------
...
int main(int ac, char *av[])
{
    ...
    Vec3d observer_max_stress_location = Vec3d(5.91335, 2.07964, 5.41942) * length_scale;
    Vec3d observer_max_displacement_location = Vec3d(3.6856, -0.0496905, 0.17474) * length_scale;
    StdVec<Vecd> stress_observer_location = {observer_max_stress_location, observer_max_displacement_location};
    ObserverBody stress_observer(sph_system, "StressObserver");
    stress_observer.generateParticles<ObserverParticles>(stress_observer_location);
    ...
    ContactRelation stress_observer_contact(stress_observer, {&wall_boundary});
    //----------------------------------------------------------------------
    ...
    ConstrainGeometryAlignedBox inlet_boundary(wall_boundary, "InletBoundary",
    AlignedBox(xAxis, Transform(Rotation3d(inlet_emitter_rotation), Vec3d(inlet_buffer_translation)), inlet_fix_half));

    ConstrainGeometryAlignedBox outlet_large_boundary(wall_boundary, "OutletLargeBoundary",
    ...
    ConstrainGeometryAlignedBox outlet_small_boundary(wall_boundary, "OutletSmallBoundary",
    ...
    // FlowRateCalculator
    //----------------------------------------------------------------------
    Vec3d inlet_flow_rate_translation = Vec3d(1.5611, 5.8559, -30.8885) * length_scale + inlet_normal * 2.0 * resolution_ref;
    Rotation3d inlet_flow_rate_rotation = inlet_emitter_rotation;
    Vec3d inlet_flow_rate_half(2.0 * resolution_ref, 3.5 * length_scale, 3.5 * length_scale);
    ...
    Vec3d outlet_large_flow_rate_half(1.0 * resolution_ref, 3.0 * length_scale, 3.0 * length_scale);
    ...
    Vec3d outlet_small_flow_rate_translation = Vec3d(9.0220, 0.9750, 18.6389) * length_scale - outlet_small_normal * 0.5 * resolution_ref;
    ...
    RegressionTestDynamicTimeWarping<ObservedQuantityRecording<Real>> write_point_pressure("Pressure", velocity_observer_contact);
    RegressionTestDynamicTimeWarping<ObservedQuantityRecording<Vecd>> write_wall_max_displacement("Position", stress_observer_contact);
    RegressionTestDynamicTimeWarping<ObservedQuantityRecording<Real>> write_wall_max_stress("VonMisesStress", stress_observer_contact);
    ...
    while (physical_time < end_time)
    {
        ...
        write_point_velocity.writeToFile(number_of_iterations);
        write_point_pressure.writeToFile(number_of_iterations);
        write_total_viscous_force_from_fluid.writeToFile(number_of_iterations);
        write_wall_max_stress.writeToFile(number_of_iterations);
        write_wall_max_displacement.writeToFile(number_of_iterations);
        body_states_recording.writeToFile();
