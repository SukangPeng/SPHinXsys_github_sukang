...
#include "Z_test_3d_carotid_steady_v1.h"
#include "sphinxsys.h"
using namespace SPH;
//-----------------------------------------------------------------------------------------------------------
...
int main(int ac, char *av[])
{
   ...
   // wall_boundary.defineAdaptationRatios(1.15, 2.0);
    ...
    // Cut Position Test
     ...
     Vec3d delete_half1(3.0 * dp_0, 3.0 * dp_0, 3.0 * dp_0);
     BodyAlignedBoxByCell outlet_detection_box1(wall_boundary, AlignedBox(xAxis, Transform(Vec3d(0.007, -0.00065, -0.0315)), delete_half1));

     Vec3d delete_half2(0.1 * dp_0, 0.1 * dp_0, 0.1 * dp_0);
     BodyAlignedBoxByCell outlet_detection_box2(wall_boundary, AlignedBox(xAxis, Transform(Vec3d(-0.008, -0.0073, 0.0255)), delete_half1));


     RealBody test_body_delete2(
         sph_system, makeShared<AlignedBoxShape>(xAxis, Transform(Vec3d(-0.008, -0.0073, 0.0255)), delete_half2, "TestBodyDelete1"));
     test_body_delete2.generateParticles<BaseParticles, Lattice>();
     BodyStatesRecordingToVtp write_body_delete2_to_vtp(test_body_delete2);
     write_body_delete2_to_vtp.writeToFile(0);
     ...
     SimpleDynamics<DeleteParticlesInAlignedBoxByCell> particles_detection1(outlet_detection_box1);
     SimpleDynamics<DeleteParticlesInAlignedBoxByCell> particles_detection2(outlet_detection_box2);
     ...
     // FlowRateCalculator
     //----------------------------------------------------------------------
     Vec3d inlet_flow_rate_translation = Vec3d(-1.78289736, 3.27264885, -26.86909925) * length_scale + inlet_normal * 2.0 * dp_0;
     Rotation3d inlet_flow_rate_rotation = inlet_emitter_rotation;
     Vec3d inlet_flow_rate_half(2.0 * dp_0, 3.5 * length_scale, 3.5 * length_scale);
     ...
     Vec3d outlet_large_flow_rate_translation = Vec3d(-6.23725814, -3.03319895, 27.2139394) * length_scale + outlet_large_normal * 5.0 * dp_0;
     Rotation3d outlet_large_flow_rate_rotation = outlet_large_emitter_rotation;
     Vec3d outlet_large_flow_rate_half(1.0 * dp_0, 3.0 * length_scale, 3.0 * length_scale);
     ...
     Vec3d outlet_small_flow_rate_translation = Vec3d(5.70464968, -1.65338904, 24.10545992) * length_scale + outlet_small_normal * 5.0 * dp_0;
     Rotation3d outlet_small_flow_rate_rotation = outlet_small_emitter_rotation;
     Vec3d outlet_small_flow_rate_half(1.0 * dp_0, 2.5 * length_scale, 2.5 * length_scale);
