...
std::string vessel_wall_path = "./input/vessel_test_narrow2.stl";
//----------------------------------------------------------------------
...
{
    Real target_time_; // 力达到最大值的时间
    Real growth_rate_; // 控制增长速率
...
class StartupRadialForce : public RadialForce
{
  ...
  public:
    ...
    Real GetScalingFactor(Real physical_time) const
    {
        if (physical_time < target_time_)
        {
            return sin(PI / 2 * (physical_time / target_time_)); // 平滑增长
        }
        return 1.0; // 达到目标时间后，保持最大力
    }
...
{
    Real target_time_; // 目标时间，在这个时间内线性增长到最大力
  ...
  public:
    ...
    // **修改 InducedAcceleration 方法，使力在 0.02s 内增长**
    Vecd InducedAcceleration(const Vecd &global_position, Real physical_time = 0.0) const 
    {
