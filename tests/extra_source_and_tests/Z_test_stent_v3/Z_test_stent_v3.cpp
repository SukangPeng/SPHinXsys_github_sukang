...
int main(int ac, char *av[])
{
    ...
    Real radial_force_magnitude = 1.5e6; // N/mass
    Real force_increase_duration = 0.0001; // 0.02s 内增大
    ...
    // 改成你的自定义类 CustomContactFactorSummation 和 CustomContactForce
    //InteractionDynamics<CustomContactFactorSummation> stent_update_contact_density(stent_vessel_contact);
    //InteractionDynamics<CustomContactFactorSummation> vessel_update_contact_density(vessel_stent_contact);
    //InteractionWithUpdate<CustomContactForce> stent_compute_solid_contact_forces(stent_vessel_contact);
    //InteractionWithUpdate<CustomContactForce> vessel_compute_solid_contact_forces(vessel_stent_contact);
    ...
    while (physical_time < end_time && !stop_simulation)
    {
        ...
        while (integration_time < output_period && !stop_simulation)
        {
            ...
            // 检测是否达到了支架扩张条件
            if (delta_y >= 4.3 * length_scale && delta_z >= 4.3 * length_scale)
            {
                std::cout << "========================================\n";
