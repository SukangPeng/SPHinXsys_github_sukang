...
//----------------------------------------------------------------------
std::string stent_path = "./input/stent_VDA_mm_F.stl";
//std::string stent_path = "./input/stent_mm.stl";
//----------------------------------------------------------------------
...
Real resolution_ref = 0.3 * length_scale;                                                             /**< 初始参考粒子间距，单位：m (米) */
Real diameter = 0.004;                                                                                      /**< 血管外径，单位：m (米) */
Vec3d domain_lower_bound = Vec3d(-15.0, -5.0, -15.0) * length_scale;                                   /**< 系统域的下边界，单位：m (米) */
Vec3d domain_upper_bound = Vec3d(22.0, 5.0, 15.0) * length_scale;                                     /**< 系统域的上边界，单位：m (米) */
...
Real physical_viscosity_vessel = 10.0; /**< 血管壁的物理粘度，单位：Pa·s (帕·秒) */
//----------------------------------------------------------------------
...
class RadialForce
{
  protected:
    ...
    Vecd center_line_direction_;

  public:
    RadialForce(Real magnitude, int axis)
        : reference_acceleration_(Vecd::Zero()), axis_(axis),
          rotation_matrix_(Mat3d::Identity()), center_line_direction_(Vecd::Zero())
    {
        reference_acceleration_[axis_] = magnitude;
        center_line_direction_[axis_] = 1.0;
    }

    void updateRotationMatrix(const Mat3d &rotation_matrix)
    {
        rotation_matrix_ = rotation_matrix;
        center_line_direction_ = rotation_matrix_ * center_line_direction_;
    }

    Vecd InducedAcceleration(const Vecd &position = Vecd::Zero(), Real physical_time = 0.0) const
    {
        Vecd radial_direction = position - position.dot(center_line_direction_) * center_line_direction_;
        radial_direction.normalize();
        return reference_acceleration_.norm() * radial_direction;
    }

    Vecd GetCurrentForce() const
    {
        return reference_acceleration_.norm() * center_line_direction_;
    }
