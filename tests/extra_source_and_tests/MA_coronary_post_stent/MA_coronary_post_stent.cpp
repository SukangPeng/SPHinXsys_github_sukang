/**
 * @file     coronary_artery_simulation.cpp
 * @brief    Main simulation file for coronary artery blood flow
 * @details  Implements SPH system setup, boundary conditions, and numerical methods
 * @author   Sukang Peng
 */
#include "MA_coronary_post_stent.h"
#include "sphinxsys.h"
using namespace SPH;
//-----------------------------------------------------------------------------------------------------------
...
int main(int ac, char *av[])
{
    ...
    //----------------------------------------------------------------------
    SolidBody wall_boundary(sph_system, makeShared<WallBoundary>("VesselWall"));
    wall_boundary.defineAdaptationRatios(1.0, 1.0);
    ...
    //water_block.defineAdaptationRatios(1.0, 1.0);
    ...
    // FlowRateCalculator
    //----------------------------------------------------------------------
    Vec3d inlet_flow_rate_translation = Vec3d(0, 0, 0) * length_scale + inlet_normal * 1.0 * resolution_ref;
